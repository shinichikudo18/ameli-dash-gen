...
try:
    ...
    def snmp_walk_map(oid, host=SNMP_HOST, community=SNMP_COMMUNITY):
        data = {}
        for item in snmp_walk(oid, host=host, community=community):
            full_oid = item.get("oid", "")
            value = item.get("value")
            if full_oid.startswith(f"{oid}."):
                index = full_oid[len(oid) + 1:]
            else:
                index = full_oid.split(".")[-1]
            data[index] = value
        return data
...
def get_sdwan_links():
    ...
    if SNMP_AVAILABLE:
        try:
            health_base = "1.3.6.1.4.1.12356.101.4.9.2.1"
            monitor_base = "1.3.6.1.4.1.12356.101.4.8.2.1"

            health_names = snmp_walk_map(f"{health_base}.2")
            health_states = snmp_walk_map(f"{health_base}.4")
            health_latency = snmp_walk_map(f"{health_base}.5")
            health_jitter = snmp_walk_map(f"{health_base}.6")
            health_loss = snmp_walk_map(f"{health_base}.9")
            health_bw_in = snmp_walk_map(f"{health_base}.11")
            health_bw_out = snmp_walk_map(f"{health_base}.12")
            health_bw_bi = snmp_walk_map(f"{health_base}.13")
            health_ifname = snmp_walk_map(f"{health_base}.14")

            monitor_names = snmp_walk_map(f"{monitor_base}.2")
            monitor_states = snmp_walk_map(f"{monitor_base}.3")
            monitor_latency = snmp_walk_map(f"{monitor_base}.4")
            monitor_jitter = snmp_walk_map(f"{monitor_base}.5")
            monitor_loss = snmp_walk_map(f"{monitor_base}.8")
            monitor_bw_in = snmp_walk_map(f"{monitor_base}.10")
            monitor_bw_out = snmp_walk_map(f"{monitor_base}.11")
            monitor_bw_bi = snmp_walk_map(f"{monitor_base}.12")

            def normalize_state(value):
                raw = str(value).strip().lower()
                if raw in {"0", "alive", "up", "online"}:
                    return "Up"
                if raw in {"1", "dead", "down", "offline"}:
                    return "Down"
                if raw in {"standby", "2"}:
                    return "Standby"
                return "Unknown"
            ...
            def health_score(state, latency, jitter, loss):
                if state == "Down":
                    return 0
                score = 100.0
                if latency is not None:
                    score -= min(latency / 5.0, 35)
                if jitter is not None:
                    score -= min(jitter / 2.0, 20)
                if loss is not None:
                    score -= min(loss * 2.5, 40)
                return max(0, round(score))

            monitor_by_name = {}
            for idx, name in monitor_names.items():
                monitor_by_name[normalize_key(name)] = idx

            indices = sorted(set(
                list(health_names.keys()) +
                list(health_states.keys()) +
                list(health_latency.keys()) +
                list(health_jitter.keys()) +
                list(health_loss.keys()) +
                list(health_bw_in.keys()) +
                list(health_bw_out.keys()) +
                list(health_bw_bi.keys()) +
                list(health_ifname.keys())
            ), key=lambda x: (len(x), x))

            for idx in indices:
                name = health_names.get(idx) or f"Link-{idx}"
                monitor_idx = monitor_by_name.get(normalize_key(name))
                state = normalize_state(health_states.get(idx, monitor_states.get(monitor_idx, "")))
                latency = to_float(health_latency.get(idx, monitor_latency.get(monitor_idx)))
                jitter = to_float(health_jitter.get(idx, monitor_jitter.get(monitor_idx)))
                loss = to_float(health_loss.get(idx, monitor_loss.get(monitor_idx)))

                result.append({
                    "name": name,
                    "interface": health_ifname.get(idx, "Unknown"),
                    "state": state,
                    "latency": f"{latency:.1f}" if latency is not None else "N/A",
                    "jitter": f"{jitter:.1f}" if jitter is not None else "N/A",
                    "packet_loss": f"{loss:.1f}" if loss is not None else "N/A",
                    "bandwidth_in": health_bw_in.get(idx, monitor_bw_in.get(monitor_idx, "N/A")),
                    "bandwidth_out": health_bw_out.get(idx, monitor_bw_out.get(monitor_idx, "N/A")),
                    "bandwidth_bi": health_bw_bi.get(idx, monitor_bw_bi.get(monitor_idx, "N/A")),
                    "sla": health_score(state, latency, jitter, loss),
                })
        except Exception as e:
    ...
    if not result:
        result = [
            {"name": "ISP1-Primary", "interface": "wan1", "state": "Up", "latency": "12.5", "jitter": "2.3", "packet_loss": "0.1", "bandwidth_in": "N/A", "bandwidth_out": "N/A", "bandwidth_bi": "N/A", "sla": 98},
            {"name": "ISP2-Backup", "interface": "wan2", "state": "Standby", "latency": "N/A", "jitter": "N/A", "packet_loss": "N/A", "bandwidth_in": "N/A", "bandwidth_out": "N/A", "bandwidth_bi": "N/A", "sla": 85},
        ]
